...
def train_loop(process_idx, env, agent, steps, outdir, counter,
    ...
    if eval_env is None:
        eval_env = env
        eval_env.env.setOutputFilePath(outdir)
        eval_env.env.setProcessIdx(process_idx)
        eval_env.env.init_output()
    try:

        total_r = 0
        episode_r = 0
        global_t = 0
        env.env.setGlobal_t(global_t)
        eval_env.env.setGlobal_t(global_t)
        local_t = 0
        global_episodes = 0
        obs = env.reset()
        r = 0
        ...
        episode_len = 0
        successful = False
        env.env.setOutputFilePath(outdir)
        env.env.setProcessIdx(process_idx)
        env.env.init_output()
        ...
        while True:
            ...
            #if global_t % 10**5 == 0 : env.env.output()
            if done or episode_len == max_episode_len:
                ...
                with counter.get_lock():
                    ...
                    env.env.setGlobal_t(global_t) # FR: update global t counter in  the workers
...
def train_agent_async_wishart(outdir, processes, make_env,
                      profile=False,
                      steps=8 * 10 ** 7,
                      eval_interval=10 ** 6,
                      eval_n_runs=10,
