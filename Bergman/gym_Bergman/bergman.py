...
class Bergman(gym.Env):
	...
	def second_init(self): # after nmod, sigma, eps are set, can run this
		...
		self.action_space = spaces.Discrete(2*self.nmod + 2)
		self.observation_space = spaces.Box(low=int(self.min_var), high=int(self.max_var), shape=(self.nmod + 1, 1))
	...
	def random_wishart(self): # pos def metric
		...
		A = np.array([np.random.normal(loc = 0, scale = self.vars[pp],size = self.nmod + 1)for pp in range(self.nmod + 1)])
